...
class WebSearchAdapterTest {
    ...
    @Test
    @DisplayName("Should return empty list when response body is null / 204 No Content")
    void search_whenResponseBodyIsNull_returnsEmptyList() {
        ExchangeFunction exchangeFunction = request -> Mono.just(
    ...
    @Test
    @DisplayName("Should return empty list when SerpAPI returns HTTP 500 error")
    void search_whenHttpServerError_returnsEmptyList() {
        ExchangeFunction exchangeFunction = request -> Mono.just(
                ClientResponse.create(HttpStatus.INTERNAL_SERVER_ERROR)
                        .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                        .body("{\"error\": \"SerpAPI internal error\"}")
                        .build()
        );
    ...
    @Test
    @DisplayName("Should return empty list when WebClient throws exception during call")
    void search_whenExceptionThrown_returnsEmptyList() {
